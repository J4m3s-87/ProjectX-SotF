...
| **Loot Sync** (LootSyncEvent + LootEventListener)                      | **10.5KB**  | 2     | Server-authoritative loot tracking — client→server collection reports via `debugCommand` Bolt event |
...
| **LootRespawn**         | 50KB+ (3 files)    | Full C# on [GitHub](https://github.com/laserman120/SOTF-Mod-LootRespawnControl) | **5× larger**    | Integer hash replacing MD5 + full container respawn (openable + breakable) + server-authoritative tracking via `debugCommand` Bolt event (silent C→S) with client→server reporting |
...
- ~~**Multiplayer loot sync**~~ — **Now implemented** (March 08, 2026): Server-authoritative tracking via `debugCommand` Bolt event for silent C→S transport (Pattern #28). Clients report ground pickup collections to the server via `LootEventListener` (GlobalEventListener). Container events fire server-side directly. Server maintains central tracker and persists to disk

Our multiplayer sync uses the `debugCommand` built-in Bolt event for C→S transport — a proven silent channel used by SimpleNetworkEvents, Shops, and Banking mods — rather than per-pickup networking.

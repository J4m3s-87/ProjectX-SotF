...
| **LootRespawn**         | 50KB+ (3 files)    | Full C# on [GitHub](https://github.com/laserman120/SOTF-Mod-LootRespawnControl) | **5× larger**    | Integer hash replacing MD5 + full container respawn (openable + breakable) + server-authoritative tracking via dedicated `LootSyncEvent` NetEvent protocol with full/incremental client sync |

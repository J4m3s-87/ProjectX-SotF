...
| **LootRespawn**         | 55KB+ (3 files)    | [Full C# (GitHub)](https://github.com/laserman120/SOTF-Mod-LootRespawnControl) — Harmony/, Networking/, `PickUp.cs`, `BreakableObject.cs`     | **5× larger**    | GLaDOS's version tracks pickups + breakables via `PickUp.Collect`/`BreakableObject.OnBreak`, has chunked networking with ACK flow control (`ConfigDataEvent`/`LootDataEvent`). Project X adds openable container tracking (`ContainerItemSpawner.OpenContainer`), integer hash replacing MD5, server-authoritative loot sync via dedicated NetEvent protocol, complete suppression/restoration system (scale+GUI+colliders+PickUp component) |

...
| Module                  | Project X          | Original        | Size Diff        | Runs On                       | Key Architectural Difference                                                                                                           |
| ----------------------- | ------------------ | --------------- | ---------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| **UI**                  | 148.8KB (4 files)  | 113.4KB (17 f)  | +31%             | Owner + Client (limited)      | Studied AxelModMenu for concepts. Completely different IMGUI rendering, permission-aware panels. Client panels gated by `#if !CLIENT`  |
| **RaidCustomizer**      | 99.6KB (11 files)  | 76KB (16 files) | +31%             | All tiers (partial on Server) | Most closely based on original. Retained core event logic, added 58KB of new code. Server runs scheduling; stat overrides on O+C       |
| **DedicatedSuperuser**  | 102.6KB (11 files) | 33KB (GitHub)   | **3.1× larger**  | All tiers                     | Adds RBAC, permission-aware GUI, remote `/px` command bridge. Server-side command dispatch + Client-side chat routing                  |
| **ScaryCross**          | 70.4KB (3 files)   | 16KB (GitHub)   | **4.4× larger**  | Owner + Client                | `#if !SERVER` — demon detection + fire VFX. Server has no visual scene to attach MonoBehaviours. Config pushed via ConfigSync          |
| **BroadcastMessage**    | 66.2KB (4 files)   | 7KB             | **9.5× larger**  | Server + Owner                | Discord rich embeds with zero-dependency JSON + in-game welcome system. Excluded from Client csproj                                    |
...
| **Inventory**           | 1.8KB (1 file)     | 14.4KB (4 f)    | condensed        | All tiers                     | No tier guards — item data modification runs on all editions. Config pushed by server                                                  |

**Every single module with a counterpart is architecturally different** — most are 2–9× the size. Code does not grow by copying.

Modules marked `#if !SERVER` still benefit clients on dedicated servers — the server controls all config via ConfigSync, and each Owner/Client applies the effects locally. The server doesn't need to run these modules itself because they operate on local game state (rendering, damage, UI, item data).
...
## LootRespawn — Deep Dive (55KB+ vs ~30KB Original)
...
| **Complete SuppressPickup**     | Hides model (scale=zero), deactivates `_PickupGui_` icon, disables all colliders recursively, disables `PickUp` component |
| **RestorePickup**               | Reverses all suppression mid-session when timer expires — with `scale==zero` guard to prevent giant icon bug on fresh instances |
| **GPS Locator categorisation**  | ItemId 529 added to Meds category so it uses per-category override timer instead of global fallback  |
